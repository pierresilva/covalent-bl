...
import { BlMessageConfig } from './bl-message-config';
import { BlMessageContainerComponent } from './bl-message-container.component';
import { BlMessageData, BlMessageDataFilled, BlMessageDataOptions } from './bl-message.definitions';

let globalCounter = 0; // global ID counter for messages

export class BlMessageBaseService<ContainerClass extends BlMessageContainerComponent, MessageData, MessageConfig extends BlMessageConfig> {
  ...
  createMessage(message: MessageData, options?: BlMessageDataOptions): BlMessageDataFilled {
    // TODO: spread on literal has been disallow on latest proposal
    const resultMessage: BlMessageDataFilled = {
      ...(message as {}), ...{
  ...
  private createContainer(): ContainerClass {
    ...
    overlayPane.style.zIndex = '1010'; // Patching: assign the same zIndex of bl-message to it's parent overlay panel, to the bl-message's zindex work.
...
export class BlMessageService extends BlMessageBaseService<BlMessageContainerComponent, BlMessageData, BlMessageConfig> {
  ...
  constructor(
    ...
    super(overlay, BlMessageContainerComponent, injector, cfr, appRef, 'message-');
  }

  // Shortcut methods
  success(content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
    return this.createMessage({ type: 'success', content }, options);
  }

  error(content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
    return this.createMessage({ type: 'error', content }, options);
  }

  info(content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
    return this.createMessage({ type: 'info', content }, options);
  }

  warning(content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
    return this.createMessage({ type: 'warning', content }, options);
  }

  loading(content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
    return this.createMessage({ type: 'loading', content }, options);
  }

  create(type: 'success' | 'info' | 'warning' | 'error' | 'loading' | string, content: string, options?: BlMessageDataOptions): BlMessageDataFilled {
